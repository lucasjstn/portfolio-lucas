    ...
    return (
        <div className="fixed h-[12vh] z-[10] bg-blue-950 w-full">
            <div className="flex items-center h-full justify-between w-[95%] sm:w-[90%] xl:w-[80%] mx-auto">
                        ...
                        {navLinks.map((navlink) => {
                            return (
                                <Link key={navlink.id} href={navlink.url}>
                                    {/* Use o elemento <a> */}
                                    <p className="nav__link text-white hover:text-blue-500">{navlink.label}</p>
                                </Link>
                            );
                        })}
                    </div>
                ...
                {/* Burger */}
                <HiBars3 onClick={openNav} className="w-8 h-8 cursor-pointer text-white lg:hidden"></HiBars3>
